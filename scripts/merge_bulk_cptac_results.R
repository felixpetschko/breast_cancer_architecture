...
objects_dir <- get_arg("--objects-dir", "results/bulk_rnaseq/cptac_brca/objects")
tables_dir <- get_arg("--tables-dir", "results/bulk_rnaseq/cptac_brca/tables")
diagnostics_json <- get_arg("--diagnostics-json", file.path(tables_dir, "cptac_brca_merge_diagnostics.json"))

dir.create(tables_dir, recursive = TRUE, showWarnings = FALSE)

estimate_path <- file.path(objects_dir, "deconv_estimate_cptac_brca.csv")
quantiseq_path <- file.path(objects_dir, "deconv_quantiseq_cptac_brca.csv")
epic_path <- file.path(objects_dir, "deconv_epic_cptac_brca.csv")
rectangle_path <- file.path(objects_dir, "deconv_rectangle_cptac_brca.csv")

required_files <- c(estimate_path, quantiseq_path, epic_path, rectangle_path)
missing_files <- required_files[!file.exists(required_files)]
if (length(missing_files) > 0) {
  stop("Missing CPTAC bulk deconvolution outputs:\n", paste(missing_files, collapse = "\n"))
}
...
raw_out <- file.path(tables_dir, "cptac_brca_all_methods_raw.csv")
agg_out <- file.path(tables_dir, "cptac_brca_all_methods_aggregated.csv")
...
message("CPTAC bulk merge complete.")
